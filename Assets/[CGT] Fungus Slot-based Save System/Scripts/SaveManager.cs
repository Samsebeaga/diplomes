...
namespace CGTUnity.Fungus.SaveSystem
{
    ...
    public class SaveManager : MonoBehaviour
    {
        ...
        [SerializeField] public GameSaver gameSaver;
...
#region Save-loading
        ...
        public virtual GameSaveData GetLatestSave()
        {
            if (WrittenSaves == null || WrittenSaves.Count == 0)
            {
                Debug.LogWarning("No saves available in WrittenSaves.");
                return null;
            }

            GameSaveData latestSave = null;
            DateTime latestTime = DateTime.MinValue;

            foreach (var save in WrittenSaves.Values)
            {
                if (save.LastWritten > latestTime) // Изменено с SaveTime на LastWritten
                {
                    latestTime = save.LastWritten; // Изменено с SaveTime на LastWritten
                    latestSave = save;
                }
            }
