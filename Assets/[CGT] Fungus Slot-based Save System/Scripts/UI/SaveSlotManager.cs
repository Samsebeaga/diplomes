...
namespace CGTUnity.Fungus.SaveSystem
{
    ...
    public class SaveSlotManager : MonoBehaviour
    {
        ...
        {
            List<GameSaveData> saves = new List<GameSaveData>(SaveManager.WrittenSaves.Values);

            GameSaveData existingSave = saves.Count > 0 ? saves[0] : null;

            AssignSingleSlot(existingSave);
        }
        ...
        public void AssignSingleSlot(GameSaveData data)
        {
            if (slots == null || slots.Count == 0)
            {
                Debug.LogWarning("AssignSingleSlot: No slots assigned in the inspector.");
                return;
            }

            var slot = slots[0];
            slot.SaveData = data; // или slot.AssignSave(data), если метод есть
            selectedSlot = slot;
        }
